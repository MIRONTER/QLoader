...
public class BaseAsyncImageLoader
{
    ...
    private Task<Bitmap?> LoadFromInternalAsync(string url)
    {
      try
      {
        ...
        if (uri.Scheme == Uri.UriSchemeHttp || uri.Scheme == Uri.UriSchemeHttps)
          return Task.FromResult((Bitmap?) null);
        return uri is {IsAbsoluteUri: true, IsFile: true} ? Task.FromResult<Bitmap?>(new Bitmap(uri.LocalPath)) : Task.FromResult<Bitmap?>(new Bitmap(AssetLoader.Open(uri)));
      }
      catch (Exception ex)
      {
        _logger?.Log(this, "Failed to resolve image from request with uri: {RequestUri}\nException: {Exception}", url, ex);
        return Task.FromResult((Bitmap?) null);
      }
    }
